...
class GroupModifyFragment :
    ...
    private val args by navArgs<GroupModifyFragmentArgs>()
    private val groupViewModel by viewModels<GroupViewModel>()
    private lateinit var groupModifiedInfo: GroupListResponse
    ...
    override fun init() {
        ...
        val maxList = res.getStringArray(R.array.max)
        val ageList = res.getStringArray(R.array.age)
        val regionList = res.getStringArray(R.array.region)

        val maxIndex = maxList.indexOf(args.groupInfo.groupMaxCount.toString())
        val ageIndex = ageList.indexOf(args.groupInfo.groupAgeRange.toString())
        val regionIndex = regionList.indexOf(args.groupInfo.groupRegion)
    ...
    private fun initClickListener() = with(binding){
        ...
        // 수정하기 버튼 클릭 시
        btnModifyGroup.setOnClickListener {
            ...
            if (name != "" && content != "" && startDate != "" && endDate != "") {
                ...
                groupModifiedInfo = GroupListResponse( args.groupInfo.groupSeq, name,"", args.groupInfo.groupMaster, content, 'Y',groupPwd, max,
                    region, startDate, endDate, age, child, global,'Y', 'R')

                groupViewModel.modifyGroup(args.groupInfo.groupSeq, groupModifiedInfo)
            }
